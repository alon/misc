...
class IPCache(object):
    ...
    _completions = ['redhat.com', 'tlv.redhat.com']
    ...
    def _write_db(self):
        header="""#autogenerated by mydns %s
...
""" % datetime.datetime.now()
    ...
    def call_dig(self, hostname):
        if hostname[-1:] != '.':
            hostname = hostname + '.'
        hosts = [hostname, hostname[:-1]]
        for comp in self._completions:
            hosts.append(hostname + comp)
        for host in hosts:
            ret = self._call_dig_single(host)
            if ret:
                return ret
        return False

    def _call_dig_single(self, hostname):
        ...
        answer = [x.strip() for x in lines[sections[answer_section]+1:sections[answer_section+1]] if x.strip() != '' and re.search(r, x)]
